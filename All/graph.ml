...
let cleareffacersvg bool = 
  if bool = true then (Graphics.set_color (rgb 255 255 255);Graphics.fill_rect 270 125 200 50) else ();;
...
let lireSauvegarde blob =
  ...
  |_  -> (cleareffacerchargerpartie true; Graphics.moveto 815 145;Graphics.set_color (rgb 0 0 0);Graphics.set_font "-*-fixed-medium-r-*--12-*-*-*-*-*-iso8859-1";
          Graphics.draw_string ("SAUVEGARDE CORROMPUE/NON TROUVEE");(true));;
...
let lancermenudebut bool =
  if (bool = true) then (menuDebut true ; reponseJ true; referencemenu true;(if (!nonSauvegarde = true) then (Sudoku.compteur :=  (20 +(int_of_string(!numerodifficultejeu)))) else (Sudoku.compteur := (Sudoku.compteZerogrille (!grillesudokuB))))) else ();;
            ...
            let key = read_key () in if (testkeyrange key) then
              (var := getCaseCoord (fst !pos) (snd !pos) arraycasex arraycasey;encore := false; dessineSelect !var true;  
               ...
               if !var = -1 then () else if((staticarraygrille.(!var) = 0) && !arraygrilleJ.(!var) != 0 ) then () else if (staticarraygrille.(!var)!= 0) then () else ( if (arraygrilleSol.(!var) = (int_of_char(key)-48) )
                                                                                                                                                                        ...
                                                                                                                                                                        else (  (if (key ='a') then () else (Sudoku.vie := !Sudoku.vie -1));draw_image (Image.init_image "rsc/cache_vie.ppm") 664 491;messagevie true; if !Sudoku.vie = 0 then (bool := false ;Sudoku.vie := 10; messageperdant true;)));
               if(key ='a') then ((val2 := (Sudoku.procedurealeatoire !arraygrilleJ arraygrilleSol) ); (dessineNb (!val2) (char_of_int(!arraygrilleJ.(!val2)+48));boolretourarriere := false);Sudoku.compteur := (!Sudoku.compteur -1);draw_image (Image.init_image "rsc/cache_vie.ppm") 664 491;messagevie true;
                                  ( (Sudoku.vie := !Sudoku.vie -1);draw_image (Image.init_image "rsc/cache_vie.ppm") 664 491 ;messagevie true; if !Sudoku.vie = 0 then (bool := false ;Sudoku.vie := 10; messageperdant true; (if (!nonSauvegarde = true) then (Sudoku.compteur :=  (20 +(int_of_string(!numerodifficultejeu)))) else (Sudoku.compteur := (Sudoku.compteZerogrille (!grillesudokuB)))))) ) else ();
