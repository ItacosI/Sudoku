...
while !b do
  let s = Graphics.wait_next_event [Graphics.Button_down]
  in match s.Graphics.mouse_y with
  |a when s.Graphics.mouse_y > 490 && s.Graphics.mouse_y < 520 -> if s.Graphics.mouse_x > 890 && s.Graphics.mouse_x < 910  then begin Graphics.moveto 900 500; let key = read_key () ; Graphics.draw_char key end
  |b when s.Graphics.mouse_y > 390 && s.Graphics.mouse_y < 420 -> if s.Graphics.mouse_x > 690 && s.Graphics.mouse_x < 720  then begin Graphics.moveto 710 400; let key = read_key () ; Graphics.draw_char key end
