...
let messagevie bool =
  if bool = true then ( Graphics.set_color (rgb 0 0 0);Graphics.set_font "-*-fixed-medium-r-*--24-*-*-*-*-*-iso8859-1";Graphics.set_text_size 100; Graphics.moveto 800 400;
                        Graphics.draw_string ( (string_of_int(!Sudoku.vie)) ^ "/10  vies restantes "); Graphics.set_text_size 50; Graphics.set_color (rgb 0 255 0);Graphics.set_font "-*-fixed-medium-r-*--15-*-*-*-*-*-iso8859-1") else ();;
...
let clearmenucompteur bool = 
  if bool = true then (  Graphics.set_color (rgb 255 255 255 );Graphics.fill_rect 750 470 400 30; Graphics.set_color (rgb 0 255 0)) else ();;
...
let message bool =
  if bool = true then (Graphics.set_font "-*-fixed-medium-r-*--18-*-*-*-*-*-iso8859-1";Graphics.set_color (rgb 0 0 0);Graphics.set_text_size 100; Graphics.moveto 620 600;
                       Graphics.draw_string "Appuyez sur Z pour faire apparaitre le menu des commandes et des regles" ;Graphics.moveto 730 530;Graphics.set_font "-*-fixed-medium-r-*--21-*-*-*-*-*-iso8859-1";
                       Graphics.draw_string(" Grille numero : " ^ (!numerogrille)  ^ "       Difficulte : "^ (!numerodifficultejeu));) else ();;


let messagecompteur bool = 
  if bool = true then ((Graphics.set_font "-*-fixed-medium-r-*--21-*-*-*-*-*-iso8859-1";Graphics.set_color (rgb 0 0 0 );Graphics.moveto 840 480;Graphics.draw_string ((string_of_int(!Sudoku.compteur)) ^ "/81 cases restantes"))) else ();;
          ...
          let key = read_key () in if (testkeyrange key) then
            (var := getCaseCoord (fst !pos) (snd !pos) arraycasex arraycasey;encore := false;	dessineSelect !var true;	
             ...
             if !var = -1 then () else if((staticarraygrille.(!var) = 0) && !arraygrilleJ.(!var) != 0 ) then () else if (staticarraygrille.(!var)!= 0) then () else ( if (arraygrilleSol.(!var) = (int_of_char(key)-48) )
                                                                                                                                                                      then ( tempretourarriere := Array.copy !arraygrilleJ; !arraygrilleJ.(!var) <- (int_of_char(key)-48);(dessineNb (!var) key);boolretourarriere := true;Sudoku.compteur := (!Sudoku.compteur -1);clearmenucompteur true;messagecompteur true)

                                                                                                                                                                      else (  (Sudoku.vie := !Sudoku.vie -1);clearmenuscore true;messagevie true; if !Sudoku.vie = 0 then (bool := false ;Sudoku.vie := 10; messageperdant true; (Sudoku.compteur :=  (20 +(int_of_string(!numerodifficultejeu)))))));
             if(key ='a') then ((val2 := (procedurealeatoire !arraygrilleJ arraygrilleSol) ); (dessineNb (!val2) (char_of_int(!arraygrilleJ.(!val2)+48));boolretourarriere := false);Sudoku.compteur := (!Sudoku.compteur -1);clearmenucompteur true;messagecompteur true;
                                (  (Sudoku.vie := !Sudoku.vie -1);clearmenuscore true;messagevie true; if !Sudoku.vie = 0 then (bool := false ;Sudoku.vie := 10; messageperdant true; (Sudoku.compteur :=  (20 +(int_of_string(!numerodifficultejeu)))))) ) else ();
             if (!arraygrilleJ = arraygrilleSol) then (bool := false ;Sudoku.vie := 10; messagegagnant true; (Sudoku.compteur := (81 - (!Sudoku.compteur - (20 +(int_of_string(!numerodifficultejeu)))))) ) else () )
